 ...
 */
export const rehypeImageSize = (options: any) => {
    return (tree: any) => {
        ...
        visit(tree, { type: "mdxJsxFlowElement", name: "Image" }, (node) => {
            ...
            const imageSize = getImageSize(imagePath);
            const widthAttr = node.attributes?.find((attr: any) => attr.name === "width");
            const heightAttr = node.attributes?.find((attr: any) => attr.name === "height");
